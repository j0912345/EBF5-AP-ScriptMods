package foes
{
   public class WormBook extends Foe
   {
      public static var iconInfo:Array = [2,28];
      
      public function WormBook(param1:int, param2:int = 0, param3:int = 0, param4:int = 0)
      {
         super();
         setStrings();
         graphicType = "Worm";
         ...
         SP = 1;
         gold = 6;
         dropList = [[Items.glass,15],[Items.buckle,5],[Items.silk,5]];
      ...
      public function takeTurn() : *
      {
         ...
         if(Math.random() < (isHard ? 0.4 : 0.15))
         {
            spawnFoe([WormBook,WormBook,WormBook,WormFuzzy,WormPutrid,WormCutie],this.level);
         }
         ...
         if(_loc1_ == 2 && (Boolean(status[Status.DEFEND]) || Boolean(status[Status.ENCHANTED])))
         {
            _loc1_ = 1;
         }
      ...
      {
         if(Battle.selectedTarget.status[Status.ENCHANTED])
         {
            castSpell("PaperSword");
         }
         else
         {
            castSpell("Lighten");
      ...
      public function defend1() : *
      {
         ...
         Skills.giveStatus(this,Status.ENCHANTED,3);
      }
